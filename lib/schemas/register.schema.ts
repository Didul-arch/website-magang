...
// Backend schema is needed because it's a multipart/form-data request which is just a string and file, no number
const registerFormSchemaBackend = z.object({
  fullName: z.string().min(3, { message: "Nama harus minimal 3 karakter" }),
  email: z.string().email({ message: "Email tidak valid" }),
  phoneNumber: z.string().regex(/^(\+62|62|0)8[1-9][0-9]{6,9}$/, {
    message: "Nomor telepon tidak valid",
...
export { registerFormSchema, registerFormSchemaBackend };
